...
from fun.view import console_show, view_token_div, view_chosen_heads, view_chosen_tokens, view_total_stat, \
    view_cartesian_div, view_outlier, view_total_outlier
from fun.comp_att import get_all_att_sentece, comp_divergence, comp_avg_and_std, update_matrix, get_head_matrix, \
    get_max, sort_outliers
    ...
    if not os.path.exists(freq_path) or not os.path.exists(mat_token_path):
        ...
        for token in un_tokens:
            ...
            for id_token in possible_id:
                ...
                heads_chosen, df = select_head_by_mn_mx(out_dir, name, token, n_token, mn, mx, id_token, bert_tokens,
                                                        vector)
                dic_head, dic_token = create_file_freq_and_save(heads_chosen, dic_head, dic_token, token, freq_path,
                                                                mat_token_path)
    else:
...
def select_head_by_mn_mx(out_dir, name, token, n_token, mn, mx, id_token, bert_tokens, vector='entropy'):
    ...
    heads_chosen = list(
        df.loc[(df['divergence'] >= (int(mn) / 100)) & (df['divergence'] <= (int(mx) / 100))].index.values)
