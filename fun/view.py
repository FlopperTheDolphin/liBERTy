...
def view_token_div(df,token,list_index,A,id_token):
 ...
 for i in range(12):
  for j in range(12):
   A[(i,j)] = (A[(i,j)]-min_v)/den
 
 
 console_show('max:',max_v)
 console_show('min:',min_v)
...
def view_matrix(A,col=False,typ=None,title=None):
 ...
 if typ==None:
  ...
  caxes = axes.matshow(A,cmap=typ,extent=[1,12,12,1])
...
def view_mul_matrix(arr,titles):
 fig, axs = plt.subplots(nrows=1, ncols=len(arr)+1)
...
def view_total_stat(dic):
 for index in dic.keys():
  ...
  for jsd in jsds:
   jsd = jsd[0]
   #remove token column from dic so we have only lists of std ready for boxplot   
   new_jsds.append(jsd)
   #unify all the lists into one 
  dic[index] = new_jsds 
...
def console_show_color_green(msg):
 console = Console()
 console.print(msg,style=GREEN,end=' ', highlight=False)
...
def view_smear(s,diff_jsd,n,app,jsd_app):
 f = interp1d(s, diff_jsd)
 import numpy as np

 x = np.linspace(1, n, num=400, endpoint=True)
 y = f(x)
 plt.plot(x,y,'-',app,jsd_app,'o')
 l=list()
 #l.append(derivative(f,1.5,dx=0.01))
 plt.ylim([-0.05, 0.05])  
 plt.show()
  
def view_plot(x,y,s,q):
 plt.ylim(0, 1)
 plt.plot(x,y,'-',s,q,'o') 
 plt.show()
...
def view_good_token(dic_good):
 console = Console()
 for k in dic_good.keys():
  if dic_good[k] == True:
   console_show_color_red(k)
  else:
   console_show_color_black(k) 
