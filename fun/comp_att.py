...
def get_ordered_token(att_token, bert_tokens):
    ...
    return pd.DataFrame(data=token_attention, columns=['token', 'attention']).sort_values(by='attention',
                                                                                          ascending=False)
...
def sort_outliers(l_diff, bert_tokens):
    return pd.DataFrame(data=l_diff, index=bert_tokens, columns=['att_diff']).sort_values(by='att_diff',
                                                                                          ascending=False)
